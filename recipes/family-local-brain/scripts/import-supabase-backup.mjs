...
function newestManifest(backupDir) {
  if (!backupDir || !fs.existsSync(backupDir) || !fs.statSync(backupDir).isDirectory()) return null;
  const candidates = fs.readdirSync(backupDir)
...
function validateBackupManifest(backupDir) {
...
function resolveBackupInput(inputPath) {
...
function addRowToImportPlan(plan, row) {
  plan.backup_rows += 1;
