...
Self-hosted Open Brain for the Family lane without the hosted Supabase project or the self-hosted Supabase stack. This recipe keeps the small capture/search/list HTTP shape used by OB1 workers, but replaces the cloud quota dependency with:
...
The default API bind is `127.0.0.1:8790`, so it is not exposed to the LAN unless you deliberately change `BRAIN_BIND_ADDR`.

## Status

This is the Family lane's local brain instance. It is intentionally separate from the Aegis local brain (port 8787) and Katherine local brain (port 8788).
...
```sh
curl -fsS -X POST http://127.0.0.1:8790/functions/v1/capture \
  -H "x-brain-key: $BRAIN_ACCESS_KEY" \
  -H "content-type: application/json" \
  -d '{"content":"first Family local brain smoke thought","metadata":{"source":"smoke-test"}}'

curl -fsS -X POST http://127.0.0.1:8790/functions/v1/search \
  -H "x-brain-key: $BRAIN_ACCESS_KEY" \
  -H "content-type: application/json" \
  -d '{"query":"Family local brain","match_count":5,"match_threshold":0.5}'
```

## Local MCP Client Repointing

Hosted Supabase clients that currently call the Open Brain MCP Edge Function can be repointed to the Family-local endpoint:

```text
http://127.0.0.1:8790/functions/v1/open-brain-mcp
```
...
```sh
curl -fsS -X POST http://127.0.0.1:8790/functions/v1/open-brain-mcp \
  -H "x-brain-key: $BRAIN_ACCESS_KEY" \
  -H "content-type: application/json" \
...
Dashboard clients that previously used the hosted `open-brain-rest` Supabase Edge Function can point core pages at the Family brain:

```sh
NEXT_PUBLIC_API_URL=http://127.0.0.1:8790
```
...
For clients that keep the old Edge Function path in their base URL, the Family brain also accepts hosted-shape REST aliases such as `/functions/v1/open-brain-rest/health`, `/functions/v1/open-brain-rest/count`, and `/functions/v1/open-brain-rest/search`.
...
Dashboard Agent Memory pages that previously used the hosted `agent-memory-api` Supabase Edge Function can point at the Family brain too:

```sh
AGENT_MEMORY_API_URL=http://127.0.0.1:8790
```

Use the same local `BRAIN_ACCESS_KEY` as the Agent Memory API key. The server also accepts the old suffix form, `http://127.0.0.1:8790/agent-memory-api`, for clients that keep the Edge Function-style base URL.

For clients that keep a full hosted-style base path, the Family brain also accepts `/functions/v1/agent-memory-api/...` aliases.
