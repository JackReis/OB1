...
## Ingestion and retrieval v2

The additive v2 implementation standardizes embedding metadata, deterministic source ingestion, bounded chunking, hybrid lexical/vector retrieval, and telemetry. It is disabled by default. Read [`IMPLEMENTATION-PLAN.md`](./IMPLEMENTATION-PLAN.md) before applying the migration or enabling either rollout flag.

```sh
docker compose exec -T db psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
  < db/migrations/20260724-ingestion-v2.sql
docker compose exec -T db bash /docker-entrypoint-initdb.d/02-open-brain-rpcs.sh
```

Start with `INGEST_V2_ENABLED=0` and `RETRIEVAL_V2_MODE=off`. Use `shadow` before `on` for retrieval, and restrict `INGEST_ALLOWED_SOURCES` before enabling v2 ingestion.
