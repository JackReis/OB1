...
// ─── Brain identity configuration (env-var driven) ───────────────────
// BRAIN_SERVICE_NAME: hyphenated brain name shown in /health, MCP
//   serverInfo, and audit log "service" field (e.g. "aegis-local-brain").
// BRAIN_SOURCE_SYSTEM: underscored source_system value written to DB rows
//   for reflections, ingestion_jobs/items, agent_memories, audit events.
//   Defaults to BRAIN_SERVICE_NAME with hyphens → underscores.
// BRAIN_AGENT_MEMORY_SERVICE: label returned by /agent-memory-api/health.
//   Defaults to "<BRAIN_SERVICE_NAME>-agent-memory".
const BRAIN_SERVICE_NAME = process.env.BRAIN_SERVICE_NAME || "aegis-local-brain";
const BRAIN_SOURCE_SYSTEM =
  process.env.BRAIN_SOURCE_SYSTEM || BRAIN_SERVICE_NAME.replace(/-/g, "_");
const BRAIN_AGENT_MEMORY_SERVICE =
  process.env.BRAIN_AGENT_MEMORY_SERVICE || `${BRAIN_SERVICE_NAME}-agent-memory`;
...
const EMBED_VERSION = process.env.EMBED_VERSION || `${EMBED_PROVIDER}:${EMBED_MODEL}:${EMBED_DIM}:v1`;
const EMBED_NORMALIZE = process.env.EMBED_NORMALIZE === "1";
const INGEST_V2_ENABLED = process.env.INGEST_V2_ENABLED === "1";
const RETRIEVAL_V2_MODE = ["off", "shadow", "on"].includes(process.env.RETRIEVAL_V2_MODE)
  ? process.env.RETRIEVAL_V2_MODE
  : "off";
const INGEST_ALLOWED_SOURCES = new Set(
  String(process.env.INGEST_ALLOWED_SOURCES || "*")
    .split(",")
    .map((value) => value.trim())
    .filter(Boolean)
);
const INGEST_RATE_LIMIT_PER_MINUTE = numberEnv("INGEST_RATE_LIMIT_PER_MINUTE", 120);
const INGEST_MAX_CHUNKS = numberEnv("INGEST_MAX_CHUNKS", 250);
const INGEST_CHUNK_MAX_CHARS = numberEnv("INGEST_CHUNK_MAX_CHARS", 4000);
...
function sendJson(res, status, payload) {
  res.writeHead(status, {
    ...
    "access-control-allow-headers": "authorization, x-brain-key, x-ingest-key, content-type, accept, mcp-session-id, mcp-protocol-version, x-agent-id, x-session-id, x-thread-id",
...
async function getEmbedding(text) {
  ...
  return normalizeEmbedding(validateEmbedding(embedding));
}

export function normalizeEmbedding(value) {
  if (!EMBED_NORMALIZE) return value;
  const magnitude = Math.sqrt(value.reduce((sum, item) => sum + item * item, 0));
  if (!Number.isFinite(magnitude) || magnitude === 0) {
    throw httpError(502, "embedding vector has zero magnitude");
  }
...
export function embeddingMetadata(now = new Date().toISOString()) {
  return {
    embedding_provider: EMBED_PROVIDER,
    embedding_model: EMBED_MODEL,
    embedding_dimensions: EMBED_DIM,
    embedding_version: EMBED_VERSION,
    embedding_normalized: EMBED_NORMALIZE,
    embedded_at: now,
  };
}

export function assertIngestSourceAllowed(source) {
  const allowed = INGEST_ALLOWED_SOURCES.has("*") || INGEST_ALLOWED_SOURCES.has(source);
  if (!allowed) throw httpError(403, `ingest source is not allowlisted: ${source}`);
}

export function consumeIngestRate(source, now = Date.now()) {
  const minute = Math.floor(now / 60_000);
  if (ingestRateWindows.size > 1000) {
    for (const [key, value] of ingestRateWindows) {
      if (value.minute < minute) ingestRateWindows.delete(key);
    }
  }
  const current = ingestRateWindows.get(source);
  const window = current?.minute === minute ? current : { minute, count: 0 };
  if (window.count >= INGEST_RATE_LIMIT_PER_MINUTE) {
    throw httpError(429, `ingest rate limit exceeded for source: ${source}`);
  }
  window.count += 1;
  ingestRateWindows.set(source, window);
  return window.count;
}

export function stableIngestKey({ source, sourceUri, sourceVersion, index, content }) {
  const locator = sourceUri || `${source}:inline`;
  const version = sourceVersion || contentFingerprint(content);
  return crypto
    .createHash("sha256")
    .update(`${source}\n${locator}\n${version}\n${index}`)
    .digest("hex");
...
export function staleSourceFilter(sourceUri, sourceVersion) {
  return `/thoughts?source_uri=eq.${encodeURIComponent(sourceUri)}` +
    `&or=(source_version.is.null,source_version.neq.${encodeURIComponent(sourceVersion)})`;
}
...
// Extract provenance from headers and/or body. Headers take precedence.
function buildProvenance(headers, body) {
  const headerProv = {
    agent_id: typeof headers["x-agent-id"] === "string" ? headers["x-agent-id"].trim() : null,
    session_id: typeof headers["x-session-id"] === "string" ? headers["x-session-id"].trim() : null,
    thread_id: typeof headers["x-thread-id"] === "string" ? headers["x-thread-id"].trim() : null,
  };
  const bodyProv = validateMetadata(body && body.provenance ? body.provenance : {});
  const prov = {};
  for (const key of ["agent_id", "session_id", "thread_id"]) {
    const val = headerProv[key] || bodyProv[key];
    if (val) prov[key] = val;
  }
  // Carry any extra provenance fields from the body (e.g. runtime, model, task_id)
  for (const [key, value] of Object.entries(bodyProv)) {
    if (!(key in prov) && value) prov[key] = value;
  }
  return prov;
}
...
function provenanceToMetadata(provenance) {
  const extra = {};
  if (!provenance || typeof provenance !== "object") return extra;
  if (provenance.agent_id) extra.agent_id = provenance.agent_id;
  if (provenance.session_id) extra.session_id = provenance.session_id;
  if (provenance.thread_id) extra.thread_id = provenance.thread_id;
  if (Object.keys(provenance).length) extra.provenance = provenance;
  return extra;
}

export async function handleCapture(body, provenance = {}) {
  ...
  const metadata = {
    ...
    ...provenanceToMetadata(provenance),
    ...embeddingMetadata(now),
  };
  const embedding = await getEmbedding(content);
  const ingestKey = textOrNull(body.ingest_key || metadata.ingest_key);
  const result = INGEST_V2_ENABLED
    ? await postgrestRpc("upsert_thought_v2", {
        p_content: content,
        p_embedding: embeddingToVectorLiteral(embedding),
        p_metadata: metadata,
        p_ingest_key: ingestKey || `capture:${contentFingerprint(content)}`,
      })
    : await postgrestRpc("upsert_thought", {
        p_content: content,
        p_embedding: embeddingToVectorLiteral(embedding),
        p_metadata: metadata,
      });
  ...
  return {
    ...
    source_type: metadata.source_type || metadata.source || BRAIN_SERVICE_NAME,
...
export async function handleSearch(body) {
  ...
  const matchThreshold =
    typeof body.match_threshold === "number" && body.match_threshold >= 0 && body.match_threshold <= 1
      ...
      // mxbai-embed-large compresses cosine similarity into a high, narrow band:
      // genuinely-relevant fleet matches land at ~0.71-0.73, so a 0.70 default
      // silently returns [] whenever embedding jitter dips a match below it.
      // 0.5 gives headroom (and matches the recall default used elsewhere here).
      // Callers can still pass an explicit match_threshold to tighten.
      : 0.5;
  const filter = validateMetadata(body.filter);
  const embedding = await getEmbedding(query);
  METRICS.search_requests += 1;
  const semanticMatches = await postgrestRpc("match_thoughts", {
    query_embedding: embeddingToVectorLiteral(embedding),
    match_threshold: matchThreshold,
    match_count: matchCount,
    filter,
  });
  if (RETRIEVAL_V2_MODE === "off") return { ok: true, matches: semanticMatches, mode: "semantic" };
  METRICS.retrieval_v2_requests += 1;
  let hybridMatches;
  try {
    hybridMatches = await postgrestRpc("hybrid_search_thoughts", {
      query_text: query,
      query_embedding: embeddingToVectorLiteral(embedding),
      match_count: matchCount,
      filter,
    ...
    if (RETRIEVAL_V2_MODE !== "shadow") throw error;
    return {
      ok: true,
      matches: semanticMatches,
      mode: "semantic",
      retrieval_v2: { mode: "shadow", degraded: true, error: error.message },
    };
  }
  if (RETRIEVAL_V2_MODE === "shadow") {
    return {
      ...
      retrieval_v2: {
        mode: "shadow",
        candidate_count: hybridMatches.length,
        top_ids: hybridMatches.slice(0, 5).map((row) => row.id),
      },
    };
  }
  return { ok: true, matches: hybridMatches, mode: "hybrid" };
}
...
function restFilterRows(rows, url) {
  ...
  return rows.filter((row) => {
    ...
    if (agentId) {
      const metadata = validateMetadata(row.metadata);
      const prov = validateMetadata(metadata.provenance);
      if (String(metadata.agent_id || prov.agent_id || "") !== agentId) return false;
    }
    return true;
  });
}
...
export function archiveSourceId(tableName, row, index) {
...
export function boolOrTrue(value) {
  return value !== false;
}

export function jsonObjectOrEmpty(value) {
  return value && typeof value === "object" && !Array.isArray(value) ? value : {};
}

export function jsonArrayOrEmpty(value) {
  return Array.isArray(value) ? value : [];
}

export function sideTableRecord(tableName, archiveRecord) {
...
export async function handleBackupArchive(body) {
...
export async function handleBackupArchiveCount(url) {
...
export async function handleFirstClassSideTableCount(url) {
  const tableName = url.searchParams.get("table") || "";
...
async function handleRestThoughts(url) {
  ...
  const hasFilters = ["type", "source_type", "source", "status", "quality_score_max", "importance_min", "agent_id"].some((key) =>
...
async function handleRestThoughtReflection(id, body = {}) {
  ...
  if (Object.keys(body).length) {
    ...
    await postgrestPost(
      ...
      [{
        id: reflectionId,
        source_system: BRAIN_SOURCE_SYSTEM,
...
export function splitIngestContent(content, maxChars = INGEST_CHUNK_MAX_CHARS) {
  ...
  if (current) chunks.push(current);
  return chunks.slice(0, INGEST_MAX_CHUNKS);
}
...
async function handleRestIngest(body) {
  ...
  assertIngestSourceAllowed(source);
  consumeIngestRate(source);
  const metadata = validateMetadata(body.metadata);
  const sourceUri = textOrNull(body.source_uri || metadata.source_uri);
  const sourceVersion = textOrNull(body.source_version || metadata.source_version) || contentFingerprint(content);
  const chunkKind = textOrNull(body.chunk_kind || metadata.chunk_kind) || "section";
  if (dryRun) {
    return {
      ok: true,
      dry_run: true,
      extracted: chunks.length,
      thoughts: chunks.map((chunk, index) => ({
        index,
        content: chunk,
        source_type: source,
        ingest_key: stableIngestKey({ source, sourceUri, sourceVersion, index, content: chunk }),
      })),
    };
  }
  const now = new Date().toISOString();
  const deterministicJobKey = crypto
    .createHash("sha256")
    .update(`${source}\n${sourceUri || "inline"}\n${sourceVersion}`)
    .digest("hex")
    .slice(0, 32);
  const jobId = textOrNull(body.job_id || body.id) || `ingest-${deterministicJobKey}`;
  METRICS.ingest_jobs += 1;
  await postgrestPost(
    ...
    [{
      ...
      error_message: null,
      metadata: { ...metadata, source_uri: sourceUri, source_version: sourceVersion, extracted: chunks.length },
      row_data: { ...body, source, source_uri: sourceUri, source_version: sourceVersion, extracted: chunks.length },
      created_at: now,
      updated_at: now,
    }],
    { Prefer: "resolution=merge-duplicates,return=representation" }
  ...
  const items = [];
  try {
    if (INGEST_V2_ENABLED && sourceUri) {
      await postgrestPatch(staleSourceFilter(sourceUri, sourceVersion), {
        status: "superseded",
        status_updated_at: new Date().toISOString(),
      });
    }
    for (const [index, chunk] of chunks.entries()) {
      const ingestKey = stableIngestKey({ source, sourceUri, sourceVersion, index, content: chunk });
      ...
      const itemId = `${jobId}:item:${ingestKey.slice(0, 16)}`;
      items.push({
        id: itemId,
        source_system: BRAIN_SOURCE_SYSTEM,
        source_id: itemId,
        ...
        status: "complete",
        source_ref: sourceUri ? `${sourceUri}#${index}` : `${source}#${index}`,
        error_message: null,
        metadata: { index, ingest_key: ingestKey, fingerprint: captured.fingerprint || captured.content_fingerprint || null },
        row_data: { index, content: chunk, thought_id: captured.thought_id },
        created_at: now,
        updated_at: new Date().toISOString(),
      });
    }
    METRICS.ingest_chunks += thoughts.length;
    if (items.length) {
      await postgrestPost(
...
function agentMemoryTableRow(memory) {
  return {
    id: memory.id,
    source_system: memory.source_system || BRAIN_SOURCE_SYSTEM,
...
function handleAgentMemoryHealth() {
  return {
    ok: true,
    service: BRAIN_AGENT_MEMORY_SERVICE,
...
async function mcpCapture(args) {
  ...
  if (!content) throw httpError(400, "content is required");
  const provenance = validateMetadata(args.provenance);
  ...
  return {
    tools: [
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            topic: { type: "string", description: "Filter by topic (matches metadata.topics)" },
            person: { type: "string", description: "Filter by person (matches metadata.people)" },
            days: { type: "integer", description: "Only thoughts from last N days" }
          }
        }
      },
      {
      ...
      },
      {
        ...
        description: "Save a new thought to the local Aegis brain.",
        inputSchema: {
          type: "object",
          properties: {
            content: { type: "string", description: "The thought content to capture" },
            ...
            provenance: {
              type: "object",
              description: "Provenance envelope (agent_id, session_id, thread_id, etc.)",
              properties: {
                agent_id: { type: "string", description: "ID of the agent that created this thought" },
                session_id: { type: "string", description: "Session ID for the capture" },
                thread_id: { type: "string", description: "Thread ID for the capture" }
              }
            }
          },
          required: ["content"]
...
async function handleOpenBrainMcp(body) {
  ...
  if (body.method === "initialize") {
    return {
      result: {
        ...
        serverInfo: { name: BRAIN_SERVICE_NAME, version: "1.0.0" },
        capabilities: { tools: {} },
      },
      jsonrpc: "2.0",
      id,
  ...
  try {
    ...
    return jsonRpcError(id, err.status ? -32602 : -32603, err.message);
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && (path === "/health" || path === "/functions/v1/health")) {
    return sendJson(res, 200, {
      ok: true,
      status: "ok",
      service: BRAIN_SERVICE_NAME,
      embedding_model: EMBED_MODEL,
      embedding_dim: EMBED_DIM,
      embedding_version: EMBED_VERSION,
      embedding_normalized: EMBED_NORMALIZE,
      ingest_v2_enabled: INGEST_V2_ENABLED,
  ...
  try {
    ...
    if (req.method === "POST" && (path === "/capture" || path === "/functions/v1/capture")) {
      ...
      const provenance = buildProvenance(req.headers, body);
      return sendJson(res, 200, await handleCapture(body, provenance));
    }
    ...
    if (req.method === "POST" && memoryPath === "/memories/review") {
      const body = await readJson(req);
      const syntheticUrl = new URL(url);
      if (body.workspace_id) syntheticUrl.searchParams.set("workspace_id", body.workspace_id);
      if (body.project_id) syntheticUrl.searchParams.set("project_id", body.project_id);
      return sendJson(res, 200, await handleAgentMemories(syntheticUrl, { reviewOnly: true }));
    }
...
if (isMain) {
  server.listen(PORT, "0.0.0.0", () => {
    console.log(`${BRAIN_SERVICE_NAME} listening on ${PORT}`);
  });
}
